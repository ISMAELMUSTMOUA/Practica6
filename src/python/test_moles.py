...
def leer_datos():
    data = []
    if len(sys.argv) > 1:
        fichero_nom = sys.argv[-1]  # para quedarnos con el último argumento
    else:
        raise ValueError("Se requiere un archivo de datos como argumento")
    with open(fichero_nom, "r") as f:
        for line in f:
